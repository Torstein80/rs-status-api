...
This image exposes vessel data from the RS feed over a small HTTP API so it can be consumed by Home Assistant or other systems.
...
- `/status` - one selected vessel as JSON
- `/boats` - all vessels in one consistent schema
- `/healthz` - health and cache details

---

## What this container does

The service fetches vessel data from the upstream RS API, normalizes each vessel into a consistent schema, and returns either:

- a single selected vessel from `/status`
- the full fleet from `/boats`

The app does **not** poll continuously.
It only fetches upstream data when `/status` or `/boats` is requested, and then caches the result for a short period.

The upstream feed currently includes, for each vessel, fields such as `rs`, `name`, `callsign`, `mmsi`, `state_description`, `Station`, `koordinater`, `imageUrl`, and `extendedState` with `StatusId`, `StatusText`, `ColorCode`, `StatusAarsak`, and `StatusMerknad`. citeturn866611view0
...
The preferred selector is **MMSI**.

You can choose the vessel in two ways:

1. Set a default vessel with the `VESSEL_MMSI` environment variable
2. Override it per request with a query parameter
...
Matching priority in `/status` is:
...
|---|---:|---|---|
| `VESSEL_MMSI` | No | `257246500` | Default vessel MMSI used by `/status` |
| `RS_ID` | No | unset | Optional fallback selector by RS number |
| `RS_NAME` | No | unset | Optional fallback selector by exact vessel name |
...
In normal use, you usually only need to change `VESSEL_MMSI`.
...
The `/status` response includes a small top-level summary plus the full normalized vessel data under `boat`.
...
```json
{
  ...
  "callsign": "LAGJ",
  "station": "Andenes",
  "timestamp": "2026-04-06T18:30:55.000Z",
  ...
  "fetched_at": "2026-04-06T18:30:55+00:00",
  "served_at": "2026-04-06T18:31:00+00:00",
  "stale": false
}
```

`state_description` and `status_text` are exported separately because the upstream feed uses them for different concepts. In the live feed, one is the general state and the other comes from `extendedState.StatusText`. citeturn866611view0

### `/boats`

`/boats` returns the full fleet in the same normalized schema for every vessel.
...
services:
  rs-status-api:
    ...
    environment:
      VESSEL_MMSI: ${VESSEL_MMSI:-257246500}
      LISTEN_HOST: 0.0.0.0
      LISTEN_PORT: 8080
      CACHE_SECONDS: 120
      REQUEST_TIMEOUT: 20
...
Before deploying the stack, set this variable in the Portainer UI if you want a vessel other than the default:

```text
VESSEL_MMSI=257246500
```

You can find MMSI values in the upstream vessel feed or from your own RS lookup workflow. The live feed exposes `mmsi` for each vessel. citeturn866611view0

or find the available rescue vessels here: https://aistracker.rs.no/#/
...
http://YOUR_CONTAINER_IP:8080/boats
...
curl "http://YOUR_CONTAINER_IP:8080/status?mmsi=257246500"
curl http://YOUR_CONTAINER_IP:8080/boats
```

Expected behavior:

- `/healthz` returns health and cache metadata
- `/status` returns one vessel in JSON
- `/boats` returns the full fleet in JSON
...
rest:
  - resource: "http://YOUR_CONTAINER_IP:8080/status"
    ...
    sensor:
      ...
      - name: "RS Vessel Status Group"
        unique_id: rs_vessel_status_group
        value_template: "{{ value_json.status_group }}"
...
To track a different vessel by default:
...
You can also override the default vessel per request:
...
The app fetches upstream data only when `/status` or `/boats` is requested.
...
- if the upstream fetch fails but cached data exists, the API serves the cached data and marks it as `stale: true`
